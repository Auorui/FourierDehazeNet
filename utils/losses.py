...
class dehaze_criterion(nn.Module):
    def __init__(self):
        ...
        self.criterion2 = ContrastLoss_res(ablation=False).cuda()
        # self.criterion3 = MSSSIMLoss().cuda()
        # self.criterion4 = Gradient_Loss()
    def forward(self, output, target_img, source_img):
        loss = self.criterion1(output, target_img) + \
               self.criterion2(output, target_img, source_img) * 0.1
               # self.criterion3(output, target_img) + \
               # self.criterion4(output, target_img) * 2
        return loss