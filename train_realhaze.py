...
from utils import dehaze_criterion, GradualWarmupScheduler, RealHaze115Dataset, dehaze_index, val_collate_fn, DeHazeTrainEpoch2

def parse_args(known=False):
    parser = argparse.ArgumentParser(description='Dehazy')
    # 选择哪种网络
    parser.add_argument('--model', type=str, default='DEANet', help='train net name')
    # 加载预训练权重，默认None
    ...
    parser.add_argument('--dataset_path', type=str,
                        default=r'E:\PythonProject\DehazeProject\data\RealHaze115',
                        help='dataset path')
    # 小样本采样次数，仅用于训练阶段，默认为4
    parser.add_argument('--repeat', type=int, default=4, help='Number of small sample samplings')
    # 随机种子数: 11, 42, 3407, 114514, 256
    parser.add_argument('--seed', type=int, default=11, help='Random seed number')
    # 训练轮次epochs次数，默认为500轮
    parser.add_argument('--epochs', type=int, default=300, help='Training rounds')
    # 图片大小
...
if __name__ == '__main__':
    ...
    train_dataset = RealHaze115Dataset(args.dataset_path, target_shape=args.input_shape,
                                 is_train=True, repeat=args.repeat)
    val_dataset = RealHaze115Dataset(args.dataset_path, target_shape=args.input_shape,
                                 is_train=False)
    
    train_loader = TrainDataloader(train_dataset, batch_size=args.batch_size, num_workers=0, persistent_workers=False)
    val_loader = EvalDataloader(val_dataset, batch_size=1, num_workers=1, collate_fn=val_collate_fn)
    ...
    for epoch in range(args.epochs):
        ...
        psnr, ssim = Defogging.evaluate(val_loader)
