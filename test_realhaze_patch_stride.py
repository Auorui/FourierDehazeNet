...
parser.add_argument('--model', default='dehazeformer_s', type=str, help='train net name')
parser.add_argument('--data_dir', default=r'E:\PythonProject\DehazeProject\data\RealHaze115', type=str, help='path to dataset')
parser.add_argument('--resume_training', default=r'E:\PythonProject\DehazeProject\model_pth\real_haze\dehazeformer_s.pth', type=str, help='path to models saving')
...
def get_weight_window(patch_size, power=2):
    """
    生成一个 patch_size x patch_size 的加权窗口，中心值大、边缘小，用于平滑融合。
    power 越大，窗口边缘衰减越明显。
    """
    h, w = patch_size, patch_size
    y, x = np.ogrid[-1:1:h*1j, -1:1:w*1j]
    window = np.exp(-(x**2 + y**2) * power)
    window = window.astype(np.float32)
    return torch.from_numpy(window)  # shape: [H, W]
...
def sliding_window_inference(img_tensor, model, patch_size=256, stride=128, device='cuda'):
    """
    对 img_tensor 进行滑窗推理并加权融合
    img_tensor: [C, H, W], 范围 [-1, 1]
    返回: 重建后的去雾结果 [C, H, W]
    """
    C, H, W = img_tensor.shape
    result = torch.zeros((C, H, W), dtype=torch.float32).to(device)
    weight_map = torch.zeros((1, H, W), dtype=torch.float32).to(device)

    weight_window = get_weight_window(patch_size).to(device).unsqueeze(0)  # [1, H, W]

    for y in range(0, H - patch_size + 1, stride):
        for x in range(0, W - patch_size + 1, stride):
            patch = img_tensor[:, y:y+patch_size, x:x+patch_size].unsqueeze(0).to(device)  # [1, C, H, W]

            with torch.no_grad():
                out = model(patch).clamp_(-1, 1).squeeze(0)  # [C, H, W]

            result[:, y:y+patch_size, x:x+patch_size] += out * weight_window
...
def test_real_haze_115_image(val_loader, model, device=torch.device('cuda:0'), patch_size=256):
    ...
    for k, (hazy, gt, patch_nums, filename, original_shape) in enumerate(val_loader):
        ...
        save_path = f"{output_dir}/smooth_restored_{name}"
        restored = sliding_window_inference(full_image, model, patch_size=256, stride=128, device='cuda')
        print(save_path)
        write_tensor(restored.unsqueeze(0), save_path)
